...
def block_glms(working_dir, subject, task, session, hemisphere):
    print("Running Surface Block GLMs: " + subject + " " + session + " " + task + " " + hemisphere)
...
def mixed_glms(working_dir, subject, task, session, hemisphere):
    print("Running Surface Mixed GLMs: " + subject + " " + session + " " + task + " " + hemisphere)
...
def event_glms(working_dir, subject, task, session,hemisphere):
    print("Running Surface Event GLMS: " + subject + " " + session + " " + task + " " + hemisphere)
...
def glmMaker(working_dir, subject, task, session, hemisphere, regressor, regressor_label, regressor_model, type, glm_label="", contrast=[], contrast_label=[]):
    ...
    File2 = os.path.join(working_dir, subject, 'INPUT_DATA', task, session,
                         'lpi_scale_tfMRI_' + task + session[0:3].title() + '2_PA_'+hemisphere+'.func.gii')
    INPUT = File1 + ' ' + File2

    if not glm_label == "":
        glm_label = "_"+glm_label

    if not os.path.exists(os.path.join(working_dir, subject, 'SURFACE_RESULTS', task, session + glm_label + '_' + type + '_censored')):
        os.makedirs(os.path.join(working_dir, subject, 'SURFACE_RESULTS', task, session + glm_label + '_' + type + '_censored'))
    os.chdir(os.path.join(working_dir, subject, 'SURFACE_RESULTS', task, session + glm_label + '_' + type + '_censored'))
    ...
    command = "3dREMLfit -matrix X.xmat_"+hemisphere+".1D \
    -GOFORIT 5 \
    -input '" + INPUT + "' \\"
    ...
    command = command + "-Rvar stats_var_"+subject+"_REML_"+hemisphere+".func.gii \
    -Rbuck STATS_"+subject+"_REML_"+hemisphere+".func.gii \
    -fout \
    -tout \
    -nobout \
    -verb"
...
#glmMaker('/home/mitchell/Desktop/Afni_Analysis', '150423', 'Axcpt', 'baseline', ConfigGLMs.AxcptEventGlm[1].split(), ConfigGLMs.AxcptEventTent)
