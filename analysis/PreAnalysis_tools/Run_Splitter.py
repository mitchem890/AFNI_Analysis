...
def find_and_split_evts(dir, images):
    ...
    for file in glob.glob(os.path.join(dir, f"{images[0].subject}_{images[0].task}_{images[0].session}_*.txt")):
        for image in images:
...
def find_and_split_movement(dir, images):
    ...
    for image in images:
        ...
        split_tr_lengthed_files(filename=filename, outfile=outfile,
                                run_length=image.run_length, line_start=int(first_tr_of_run))
        ...
        split_tr_lengthed_files(filename=filename, outfile=outfile,
                                run_length=int(image.run_length), line_start=int(first_tr_of_run))
