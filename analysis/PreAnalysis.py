...
def preanalysis(origin, destination, events, wave, subject, session, task, pipeline, run_volume, run_surface):
    print("Running Preanalysis on: " + subject + ' ' + wave + ' ' + session + ' ' + task)

    # If there is no INPUT_DATA folder in the subject create it
    if not os.path.exists(os.path.join(destination, subject, 'INPUT_DATA', task,session)):
    ...
    for encoding in encodings:
        ...
        if pipeline == 'hcp':
            ...
            Copy_Input_Data.copy_input_data_fmriprep(subject, wave, session, task, encoding, runNum, origin, destination, events)

    #This is independant of the encoding
    Format_Motion_Regressors.format_motion_regressors(destination, subject, session, task)  # format the motion regressors
    ...
    for encoding in encodings:
        ...
        if run_volume:
            Volume_Demean_Images.volume_demean_images(destination, subject, session, task, runNum,encoding)  # Demean the volume images
        if run_surface:
            for hemisphere in hemispheres:
                Surface_Demean_Images.surface_demean_images(destination, subject, session, task, runNum, encoding, hemisphere)  # Demean the surface images
